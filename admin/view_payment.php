...
<!-- heading -->
<h4 align="center"><b>View Payment</b></h4>
<?php
// database connection
include './../database.php';
$select_pay="SELECT * FROM payment";
$result_pay=mysqli_query($con,$select_pay);
$num=mysqli_num_rows($result_pay);
// no  payment condition (for svg)
if($num >0)
{
echo "<table align='center'class='table table-striped table-light'><thead class='thead-dark'><tr><th scope='col'>S.No</th><th scope='col'>Payment Type</th><th scope='col'>Edit</th><th scope='col'>Delete</th></tr></thead><tbody>";
while($row=mysqli_fetch_array($result_pay))
{
            $pay_id = $row['payment_id'];
            $pay_type = $row['payment_type'];
            echo "<tr><td scope='row'>$pay_id</td><td>$pay_type</td></td><td><a href='admin.php?pay_Eid=$pay_id'><i class='fa-solid fa-pen-to-square edit_color'></i></a></td><td><a onclick='del_pay()'><i class='fa-solid fa-trash del_color'></i></a></td></tr>";
}
echo "</tbody></table>";
}
else{
...
<script>
    ...
    {
    // js href link
        window.location.href='admin.php?pay_Did=<?php echo $pay_id ?>';
    }
}
</script>
